	...
	for (const child of children ?? []) {
		...
		if (steps.length > 0) {
			for (const step of steps) {
				...
				rows.push({ name: step.agent, state: step.status, ...(modelThinking ? { modelThinking } : {}), ...(activity ? { activity } : {}), ...(step.startedAt !== undefined ? { startedAt: step.startedAt } : {}) });
			}
			continue;
		}
		const modelThinking = formatModelThinking(child.model, child.thinking);
		const activity = nestedActivity(child);
		rows.push({ name: nestedRunLabel(child), state: child.state, ...(modelThinking ? { modelThinking } : {}), ...(activity ? { activity } : {}), ...(child.startedAt !== undefined ? { startedAt: child.startedAt } : {}) });
	}
...
export function collectFleetStatusEntries(state: SubagentState): FleetStatusEntry[] {
	...
	for (const control of state.foregroundControls.values()) {
		if (control.activeChildren) {
			for (const child of [...control.activeChildren.values()].sort((left, right) => left.index - right.index)) {
				...
				const nestedChildren = control.nestedChildren?.filter((nested) => nested.parentStepIndex === child.index);
				entries.push({
					...
					state: "running",
					...(nestedChildren?.length ? { nestedChildren } : control.activeChildren.size === 1 && control.nestedChildren?.length ? { nestedChildren: control.nestedChildren } : {}),
				});
			}
			continue;
		}
	...
	for (const job of state.asyncJobs.values()) {
		...
		for (const [offset, step] of steps.entries()) {
			...
			const nestedChildren = step.children?.length
				? step.children
				: job.nestedChildren?.filter((nested) => nested.parentStepIndex === index);
			entries.push({
