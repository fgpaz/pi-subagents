...
describe("async compaction resume", () => {
	it("continues an interactive parent after compaction while async work is active", () => {
		const script = String.raw`
			...
			if (sent.length !== 1 || sent[0].options?.triggerTurn !== true || sent[0].message?.customType !== "subagent-compaction-resume" || sent[0].message?.display !== false) throw new Error(JSON.stringify(sent));
			sent.length = 0;
			...
			ctx.hasUI = false;
			events.emit("subagent:async-started", { id: "no-ui-running", pid: 1, sessionId: "compact-session", mode: "single", agent: "worker", asyncDir: path.join(os.tmpdir(), "pi-compaction-test-no-ui") });
			handlers.get("session_compact")();
			if (sent.some((entry) => entry.message?.customType === "subagent-compaction-resume")) throw new Error("resumed without UI");
		`;
